...
<body>
  ...
  <a href=top></a>
          <div id="image">
             <img src="css/img/111.jpg"  width="1200px" height="400px">
         </div>
         <hr>
  <h2 style="display:inline;">Human Development Books </h2>
  @if(Auth::check() &&( Auth::user()->hasRole(['Admin','Editor']) ||Auth::user()->hasRole('Editor')))
  <small>To Upload some Books....<a href="posts/upload">Upload</a></small>
  @endif
 <br><br>
  ...
  <div class="row">
      ...
      <div class="row">
        <div class="row">
            @foreach ($data as $data)
              <div class="col-md-4">
                 <div class="col-mb-7"  style="min-width:17rem;">
                        {{-- <img src="{{asset('storeage/books/'.$data->image)}}" alt="" height="200"> --}}
                    ...
                    <div class="card-body mb-5">
                          ...
                          <hr>
                          <a href="{{'/view/' . $data->id}}" class="btn btn-primary float-left mr-2"> View</a>

                          @if(Auth::check() &&( Auth::user()->hasRole(['Admin','Editor']) ||Auth::user()->hasRole('Editor')))
                            <form action="/pages/{{$data->id}}/delete" method="POST" >
                              @csrf
                              @method('DELETE')
                          ...
                          @else
                              @auth
                              <a href="{{'/pages/download/' . $data->file}}" class="btn btn-success float-left"> Download</a>
                              @endauth
                          @endif
                        </div>
                  </div>
   ...
   <div class="col-md-3">
     <div class="card ml-3" style="max-width:10rem; ">
          ...
          <div class="card-body">
            <p class="card-text">All Books: {{$count}}</p>
          </div>
     </div>
  </div>
</div>
